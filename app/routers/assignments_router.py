...
async def fetch_assignment(
    ...
    logger.info("Fetching assignment", extra={"assignment_id": assignment_id, "action": "fetch"})
    ...
    logger.error("Assignment not found", extra={"assignment_id": assignment_id, "action": "fetch_failed"})
...
async def create_update_assignment(
    ...
    """
    logger.info(
        "Processing assignment upsert",
        extra={"project_code": payload.project_code, "employee_email": payload.employee_email, "action": "upsert"},
    )
    await check_headers(user)
    ...
    if project_rtn is None:
        logger.error("Project not found", extra={"project_code": payload.project_code, "action": "upsert_failed"})
    ...
    if employee_rtn is None:
        logger.error("Employee not found", extra={"employee_email": payload.employee_email, "action": "upsert_failed"})
        raise HTTPException(404, "Unable to find employee by supplied email.")

    assignstmt = select(Assignment).filter(
        and_(Assignment.project_id == project_rtn.id, Assignment.employee_id == employee_rtn.id)
    )
    assignment_rtn = (await db.execute(assignstmt)).scalar_one_or_none()
    ...
    if assignment_rtn is None:
        logger.info(
            ...
            extra={"project_id": project_rtn.id, "employee_id": employee_rtn.id, "action": "create"},
        )
